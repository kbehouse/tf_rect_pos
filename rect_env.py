...
class Viewer(pyglet.window.Window):
    ...
    def __init__(self, width, height, target_rect_pos, rect_w, rect_h, predict_rect_pos, mouse_in):
        ...
        self.predict_label = pyglet.text.Label(text="Predict (Green): ", x=10, y=height -40 , font_size=8.0)
        self.dis_label = pyglet.text.Label(text="Distance(Err): ", x=10, y=height -60 , font_size=8.0)
    ...
    def _update_rect(self):
        ...
        self.predict_label.text = "Predict (Green): " + str(self.predict_rect_pos)
        
        dis =  np.sqrt(np.sum((self.target_rect_pos-self.predict_rect_pos)**2))
        self.dis_label.text = "Distance(Err): " + str(dis)
    ...
    def get_last_filename(self):
        file_num=str(__class__.frame_count-1).zfill(5)
        return  self.output_dir + "/frame-"+file_num+'.png'
...
class RectEnv(object):
    ...
    def set_predict_rect_pos(self, pos):
        self.predict_rect_pos[:] = pos
    ...
    def recreate_dir(self, tar_dir):
        ...
        self.viewer.set_ouput_dir(tar_dir)


    def get_last_filename(self):
        return self.viewer.get_last_filename()
